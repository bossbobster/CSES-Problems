...
bitset<128> cols[101];
bitset<128> row;
static inline void add(int col, int val)
{
    cols[col][val] = 1;
}
static inline int get(int col)
{
    bitset<128> both = ~(row | cols[col]);
    ull lower = (both & bitset<128>(0xFFFFFFFFFFFFFFFFULL)).to_ulong();
    ull upper = (both >> 64).to_ulong();
    if(lower == 0)

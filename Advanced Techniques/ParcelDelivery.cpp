...
bitset<1010> visited;
int dist[1010], upDist[1010], len[1010];
edge par[1010];
queue<int> q;
int main()
{
    input();
    int n, m, k, st, en, a, b, c, d, curN, curD, curF, flow = 0, ans = 0;
    edge curP, it;
    pq tmp;
    bool good = false;
    cin >> n >> m >> k;
    ...
    {
        cin >> a >> b >> c >> d; a--; b--;
        adj[a].push_back({b, c, d, 0, 0, i}); adj[b].push_back({a, 0, -d, 0, 0, i});
    }
    for(int i = 0; i < n; i ++)
        dist[i] = 2000000000;
    dist[st] = 0; q.push(st);

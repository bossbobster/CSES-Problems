...
//for one bit positions, multiply negative by positive:
//0 0 0 0 bit string
//bit string 0 0 0 0

ifstream fin("/Users/bobby/Downloads/test_input (2).txt");
...
int main()
{
    ...
    inv = true;
    fft(faF);
    for(int i = k/2; i < k/2+st.length()-1; i ++)
        cout << (ll)round(abs(faF[i].first)) << ((i==(int)k/2+st.length()-2)?'\n':' ');
}
